...
config_args = parser.add_argument_group(
    "PolyA read options", "Set read-level configs for defining polyA reads"
)
config_args.add_argument(
    "--minMAPQ",
    dest="minMAPQ",
...
config_args.add_argument(
    "--cell_barcode_tag",
    dest="corrected_cell_barcode_tag",
    type=str,
    default="CB",
...
running_args = parser.add_argument_group(
    "Running", "For changing how this script runs. Stop/start on polyA step e.t.c"
)
# running_args.add_argument('-v', '--version', dest='print_version', action='store_true', default='False',
#                    help="Print version and exit.")
running_args.add_argument(
    "-t",
    "--threads",
    dest="threads",
    type=int,
...
def main():
    ...
    input_from_dir = (
        os.path.isdir(args.input[0]) or len(input_bams) > 1
    )  # Already sanity checked, if dir, only one dir
    ...
    for input_bam in input_bams:
        quick_bam_check(
            input_bam, args.corrected_cell_barcode_tag, args.umi_tag, args.minMAPQ
        )
    ...
    count_from_annotated_bams(
        peak_anno_bams=anno_bams,
        input_from_dir=input_from_dir,
...
def process_bams_to_polyA_bam(
    input_bams, polyA_bam_root, minpolyA, minMAPQ, nonA_allowed, threads
):
    ...
    if len(input_bams) == 1:
        # make_only_polyA_bam(
        #     input_bams[0], polyA_bam_file, minpolyA, minMAPQ, nonA_allowed
        # )
        make_only_polyA_bam_parallel(
            input_bams[0], polyA_bam_file, minpolyA, minMAPQ, nonA_allowed, threads
        )
        ...
        for input_bam in input_bams:
            ...
            make_only_polyA_bam(
                input_bam, polyA_bamfile_ind, minpolyA, minMAPQ, nonA_allowed
            )
            pysam.index(
                polyA_bamfile_ind
            )  # Samtools index (was indexed already :. arleady sorted.)
    ...
    if not os.path.exists(polyA_bam_file + ".bai"):
        sys.exit(
            "No bam index made for polyA-only bam file "
            + polyA_bam_file
            + " Something broke."
        )
...
def _virtual_offset_to_gb(virtual_offset: int) -> float:
    # Extract the high-order 64 bits (the byte offset of the BGZF block)
    block_offset = virtual_offset >> 16
    # Convert the block offset to gigabytes (1 GB = 2^30 bytes)
    block_offset_gb = block_offset / (2**30)
    return block_offset_gb


def _build_offsets_strict(
    bam_file, num_map: int, num_chunks: int, offset: int = None
) -> list[int]:
    entries_per_chunk = math.ceil(num_map / num_chunks)
    bam = pysam.AlignmentFile(bam_file, "rb", threads=4)
    start_offset = bam.tell() if offset is None else offset
    bam.seek(start_offset)
    offsets = [start_offset]  # start with the beginning of the file
    contigs = bam.references

    iterator = bam.fetch(until_eof=True)
    for _ in trange(1, num_chunks):
        for _, _ in zip(trange(entries_per_chunk), iterator):
            pass
...
def _check_sorted(bam_file: str) -> bool:
    offsets = []
...
def build_offsets(bam_file: str, num_chunks: int, mode: str = "random") -> list[int]:
    df_stats = _get_bam_stats(bam_file)
    num_unmapped = df_stats["unmapped"].sum()

    num_mapped = df_stats["mapped"].sum()
    num_map = num_mapped + num_unmapped

    if mode == "strict":
        return _build_offsets_strict(bam_file, num_map, num_chunks, offset=None)
    elif mode == "random":
        if not _check_sorted(bam_file):
            raise ValueError(
                "Bam file is not sorted. This is not supported for random splitting."
            )
        if not num_unmapped == df_stats.query("contig == '*'")["unmapped"].iloc[0]:
            raise ValueError(
                "Unmapped entries are not all in the same contig. This is not supported."
            )
        if df_stats.query("contig == '*'")["mapped"].iloc[0]:
            raise ValueError(
                "Mapped entries are found in unmapped contig. This is not supported."
            )
        if not num_mapped:
            raise ValueError("No mapped entry. This is not supported.")
    else:
        raise ValueError(
            f"Unknown split mode {split_mode}. Choose from `strict` or `random`"
        )

    # Assign chunks to unmapped entries. Try to assign in proportion, but make sure
    # mapped entries at least have 1 chunk, so do unmapped entries.
    num_chunks_mapped = int(
        np.clip(
            np.round(num_chunks * num_mapped / num_map),
            1,
            num_chunks - 1 if num_unmapped else num_chunks,
        )
    )
    num_chunks_unmapped = num_chunks - num_chunks_mapped

    # find offset list for mapped entries
    df_stats_mapped = df_stats.query("contig != '*'").copy()

    df_stats_mapped["cumulative_mapped"] = df_stats_mapped["mapped"].cumsum()
    total_mapped = df_stats_mapped["cumulative_mapped"].iloc[-1]

    quantile_positions = []
    for value in [
        i / num_chunks_mapped * total_mapped for i in range(1, num_chunks_mapped)
    ]:
        # Get the row where the cumulative_mapped is just over the quantile value
        row = df_stats_mapped[df_stats_mapped["cumulative_mapped"] >= value].iloc[0]
        contig = row["contig"]
        # Estimate the start position assuming uniform distribution
        contig_length = row["length"]
        mapped = row["mapped"]
        cumulative_mapped = row["cumulative_mapped"]
        start_pos = int(
            ((value - (cumulative_mapped - mapped)) / mapped) * contig_length
        )
        quantile_positions.append((contig, start_pos, contig_length))

    offsets_mapped = []
    init_width = 10000
    f = pysam.AlignmentFile(bam_file, "rb")
    start_offset = f.tell()
    offsets_mapped.append(start_offset)
    for contig, position, contig_length in quantile_positions:
        width = init_width
        while True:
            start = position - width
            end = position + width
            if end > contig_length:
                start -= end - contig_length
                end = contig_length
            if start < 0:
                end = min(end - start, contig_length)
                start = 0
            try:
                next(f.fetch(contig, start, end))
                offset = f.tell()
            except StopIteration:
                width *= 2
                continue
            offsets_mapped.append(offset)
            break

    # find offset list for unmapped entries
    if num_unmapped:
        next(f.fetch("*"))
        offset_unmapped = f.tell()
        # minus 1 because the first entry is already read
        offsets_unmapped = _build_offsets_strict(
            bam_file,
            num_unmapped - 1,
            num_chunks_unmapped,
            offset=offset_unmapped,
        )
    else:
        offsets_unmapped = []
    f.close()

    offsets = offsets_mapped + offsets_unmapped
    # turn to GB and print
    offsets_gb = [round(_virtual_offset_to_gb(i), 1) for i in offsets]
    print(f"Offsets of {num_chunks} chunks are {offsets_gb} GB.")
    print(
        f"First {len(offsets_mapped)} chunks are for mapped entries and last "
        f"{len(offsets_unmapped)} chunks are for unmapped entries."
    )

    # if there are consecutive offsets, remove the second one
    offsets_u = np.unique(offsets).tolist()
    gbs_u = [_virtual_offset_to_gb(i) for i in offsets_u]
    if len(offsets_u) < len(offsets):
        print("Consecutive offsets found. Only retaining first ones.")
        print(f"Consequently, there are only {len(offsets_u)} chunks left.")
        print("They are:", [round(i, 1) for i in gbs_u])

    # report size of each chunk
    gbs_u.append(os.path.getsize(bam_file) / 1024**3)
    d_gbs_u = np.diff(gbs_u).tolist()
    print(f"Size of each chunk are:", [round(i, 1) for i in d_gbs_u])
    return offsets_u


def _get_bam_stats(bam_file: str, report: bool = False) -> pd.DataFrame:
    stats = subprocess.getoutput(f"samtools idxstats {bam_file}")
    # f"samtools idxstats {bam_file} | awk -F '\t' '{{s+=$3+$4}}END{{print s}}'"
    ...
    if report:
        ...
        num_unmapped = df_stats["unmapped"].sum()
        num_map = num_mapped + num_unmapped
        num_contigs = len(df_stats.query("length > 0"))
        print(
            f"The input bam file {bam_file} has:",
            f"\t{num_contigs} contigs",
            f"\t{num_mapped} mapped entries",
            f"\t{num_unmapped} unmapped entries",
            f"\tin total {num_map} entries",
            sep="\n",
        )
...
def make_only_polyA_bam_parallel(
    bam_file: str,
    polyA_bamfile: str,
    minpolyA: int,
    minMAPQ: int,
    ...
    df_stats = _get_bam_stats(bam_file, report=True)
    # this is a rough estimation.
    entries_per_chunk = math.ceil(
        (df_stats["mapped"].sum() + df_stats["unmapped"].sum()) / threads
    )   

    print("Building chunk offsets")
    offsets = build_offsets(bam_file, threads, mode=split_mode)

    def _make_only_polyA_bam_chunk(i: int):
        output_bam = os.path.join(
            os.path.dirname(polyA_bamfile), f".{os.path.basename(polyA_bamfile)}.{i}"
        )
        with (
            pysam.AlignmentFile(bam_file, "rb", threads=2) as bam,
            pysam.AlignmentFile(output_bam, "wb", template=bam, threads=2) as polyAbam,
        ):
            bam.seek(offsets[i])
            iterator = bam.fetch(until_eof=True)
            for read in tqdm(iterator, total=entries_per_chunk):
                if i < threads - 1 and bam.tell() >= offsets[i + 1]:
                    break
                strand = "-" if read.is_reverse else "+"
                readseq = read.query_sequence
                # print("strand=\t"+strand)
                ...
                is_polyA = False
                if strand == "+":
                    soft_match_size = read.query_length - read.query_alignment_end - 1
                    if soft_match_size >= minpolyA:
                        soft_match_seq = read.query_sequence[-soft_match_size:]
                        num_As = soft_match_seq.upper().count("A")
                        if num_As >= minpolyA and num_As >= (
                    ...
                    soft_match_size = read.query_alignment_start
                    if soft_match_size >= minpolyA:
                        soft_match_seq = read.query_sequence[0:soft_match_size]
                        num_As = soft_match_seq.upper().count("T")
                        if num_As >= minpolyA and num_As >= (
    ...
    print("Filtering polyA reads")
    polyA_bamfile_inds = Parallel(n_jobs=threads)(
        delayed(_make_only_polyA_bam_chunk)(i) for i in range(threads)
    )

    print("Merging polyA reads")
    merge_bams(polyA_bamfile_inds, polyA_bamfile)
    for bam in polyA_bamfile_inds:
        os.remove(bam)
...
def process_polyA_ends_to_peaks(
    polyA_bam,
    polyA_peaks_gff,
    depth_threshold,
    region_size,
    corrected_cell_barcode_tag,
    umi_tag,
    misprime_A_count,
    misprime_in,
):
    ...
    for read in samfile.fetch(until_eof=True):
        ...
        # A reversed mapping indicates the transcript is on the plus strand of the 
        # reference genome. Ploy A peaks derived from these reads will span 250bp, 
        # starting from the start of the mapping (on the reference).
        # A forward mapping, on the contrary, indicates the transcript is on the
        # minus strand of the reference genome. Ploy A peaks derived from these
        # reads will span 250bp, ending at the end of the mapping (on the reference).
    ...
    for contig in sorted(ends.keys()):
        for pos in sorted(ends[contig].keys()):
            for strand in sorted(ends[contig][pos].keys()):
                ...
                misprime = (
                    misprime_site[contig][pos][strand] >= count / 2
                )  # If at least half the reads are tagged as misprimed (I think it should be all, or none)
                if count >= depth_threshold:
                    if strand == "+":
                        regions[contig][max(pos - region_size, 0)] = (
                            count,
                            pos,
                            strand,
                            misprime,
    ...
    for contig in sorted(regions.keys()):
        ...
        for strand in ["+", "-"]:
            starts = sorted(
                x for x in regions[contig].keys() if regions[contig][x][2] == strand
            )
    ...
    for contig in sorted(regions.keys()):
        for pos in sorted(regions[contig].keys()):
            ...
            detail = 'peak="%s"; peakdepth="%d"; misprime="%s";' % (
                peak_name,
                depth,
                str(misprime),
            )
    ...
    # Summary
    print(
        "Processed %d polyA reads, %d included (had %s and %s tags)"
...
def make_peak_hits_annotated_bams(
    input_bams,
    input_from_dir,
    polyA_peaks_gff,
    peak_anno_bam_root,
    ...
    if not input_from_dir:
        ...
        for input_bam in input_bams:
            peak_anno_bams.append(
                os.path.join(peak_anno_bam_root, os.path.basename(input_bam))
            )
...
# uses UR?
def make_peak_hit_annotated_bam(
    input_bam, peaks_gff, peak_anno_bam, corrected_cell_barcode_tag, threads
):
    ...
    temp_feature_counts_dir = (
        os.path.basename(peak_anno_bam) + "_" + os.path.basename(peaks_gff) + "_temp"
    )
    try:
        ...
        print(
            "Couldn't create temporary dir for featureCounts run (already exists?)"
            + temp_feature_counts_dir
        )
    ...
    temp_unfiltered = os.path.join(
        temp_feature_counts_dir, os.path.splitext(os.path.basename(peak_anno_bam))[0]
    )  # is an outputfile
    temp_unfiltered_bam = os.path.join(
        temp_feature_counts_dir, os.path.basename(input_bam) + ".featureCounts.bam"
    )  # bam named via input file, not -o
    ...
    if not os.path.exists(peak_anno_bam) or os.stat(peak_anno_bam).st_size == 0:
        sys.exit(
            "Failed to filter featureCounts output into annotated bam with cmd:\n"
            + "samtools view -h "
            + temp_unfiltered_bam
            + " | grep -E '(^@)|(XT:Z:)' | grep -E '(^@)|("
            + corrected_cell_barcode_tag
    ...
    if not os.path.exists(peak_anno_bam + ".bai"):
        sys.exit(
            "No bam index made for file " + peak_anno_bam + " Something went wrong."
        )
...
def count_from_annotated_bams(
    peak_anno_bams, input_from_dir, counts_root, corrected_cell_barcode_tag, umi_tag
):
    ## Figure out the output.
    counts_files = list()
    if not input_from_dir:
    ...
    for n in range(0, len(peak_anno_bams)):
        ...
        ut_cmd = [
            ...
            "tag",
            "--umi-tag=" + umi_tag,
            "--cell-tag=" + corrected_cell_barcode_tag,
            "-I",
            peak_anno_bam,
            "-S",
            counts_file,
        ...
        if not ranok == 0:
            sys.exit(
                "Failed to run umi_tools correctly ("
                + str(ranok)
                + ") with cmd\n"
                + " ".join(ut_cmd)
            )
...
def check_params_ok(args):
    ...
    if args.polyA_bams and args.peak_anno_bams:  # ok, might happen for large dataset.
        sys.exit(
            "Can't specify both --polyA_bams and --peak_anno_bams. If you do have both processed already. "
    ...
    if args.peaks_gff and args.polyA_bams:
        sys.exit(
            "Can't specify both --polyA_bams and and --peaks_gff. "
...
def read_files_list_or_dir(filesin, filesuffix=".bam"):
    ...
    for afile in actual_files:
        if not os.path.exists(afile):
            sys.exit("File " + afile + " does not exist")
        if os.path.isdir(afile):
            sys.exit(
                ...
                + " in "
                + filesuffix
                + " file list (specify one dir OR list of files)"
            )
        if not afile.endswith(filesuffix):
            sys.exit("File " + afile + " does is not of expected type " + filesuffix)
        if os.stat(afile).st_size == 0:
            sys.exit("File " + afile + " is empty")
        if os.path.basename(afile) in seenit:
            sys.exit(
                "Filename '"
                + afile
                + "' is duplicated in input (all input bamfiles must have unique names)"
            )
...
def check_tools_available():
    ...
    try:
        ...
        sys.exit(
            "Could not find samtools in PATH. The samtools package should be installed and in PATH."
        )
    ...
    try:
        ...
        sys.exit(
            "Could not find umi_tools in PATH. The umi_tools package should be installed and in PATH."
        )
    ...
    try:
        ...
        sys.exit(
            "Could not find featureCounts in PATH. The subread package (v1.5.3 or later) should be installed and in PATH."
        )
    try:
        version = (
            version_info_str.decode("utf-8")
            .replace("\n", "")
            .split()[1]
            .replace("v", "")
        )  # Eeeeewwwwww.
        ...
        if StrictVersion(version) < StrictVersion("1.5.3"):
            sys.exit(
                "Found featureCounts, but an old version. Need version 1.5.3 or later for this to work."
            )
    except:
        sys.exit(
            "Coudln't parse the output of 'featureCounts -v'. The subread package (v1.5.3 or later) should be installed and in PATH."
        )
...
def quick_bam_check(bam_file, cell_tag, umi_tag, minMAPQ):
    ...
    if not seen_map_qual_ok:
        sys.exit(
            ...
            + " reads of "
            + bam_file
            + " and did not see any reads that have min map quality "
            + str(minMAPQ)
        )
